/*Implemente uma função que receba, pelo menos, uma árvore rubro-negra. Essa função deverá retornar 
um número >= 0 que represente a diferença de somatórios entre nós pretos e vermelhos. Por exemplo, 
primeiramente deve ser somado os valores dos nós de cada tipo. Em seguinda, calcular a diferença 
entre essas somas.

Input Format
Na primeira linha deve ser lido o número inteiro N, que é referente à quantidade de nós da árvore. 
Em seguida, em cada linha deve ser lido um número inteiro para ser inserido como nó na árvore (fazer 
...
19
Explanation 0
Nós pretos (NP): 4 + 2 + 1 + 3 + 6 + 5 + 7 + 9 = 37
Nós vermelhos (NV) = 8 + 10 = 18
|NP - NV| = |37 - 18| = 19
...
int soma_vermelhos(NodeVP *tree){
    if(tree != NULL)
        return (tree->color == 0)*tree->key + soma_vermelhos(tree->left) +soma_vermelhos(tree->right);    
    else
        return 0;
}
int soma_preto(NodeVP *tree){
        if(tree != NULL)
            return (tree->color == 1)*tree->key + soma_preto(tree->left) +soma_preto(tree->right);
    else
        return 0;
}
int main() {
    ...
    printf("%d", abs(soma_vermelhos(tree)-soma_preto(tree)));
